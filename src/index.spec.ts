...
describe('eslint-config-xs', () => {
  it('.ts file', async () => {
    ...
    const expected = [
      { ruleId: '@typescript-eslint/no-unused-vars', message: "'foo' is assigned a value but never used.", line: 2, column: 7 }, // prettier-ignore
      // { ruleId: '@typescript-eslint/require-await', message: "Async arrow function 'fn' has no 'await' expression.", line: 4, column: 38 }, // prettier-ignore
      // { ruleId: '@typescript-eslint/no-unsafe-call', message: 'Unsafe call of an `any` typed value.', line: 6, column: 1 }, // prettier-ignore
      // { ruleId: '@typescript-eslint/await-thenable', message: 'Unexpected `await` of a non-Promise (non-"Thenable") value.', line: 9, column: 1 }, // prettier-ignore
      { ruleId: 'unicorn/error-message', message: 'Pass a message to the `Error` constructor.', line: 14, column: 9 }, // prettier-ignore
      { ruleId: 'eqeqeq', message: "Expected '===' and instead saw '=='.", line: 12, column: 19 }, // prettier-ignore
    ];
  ...
  it('.tsx file', async () => {
    ...
    const expected = [
      ...
      { ruleId: '@stylistic/semi', message: 'Missing semicolon.', line: 8, column: 34}, // prettier-ignore
